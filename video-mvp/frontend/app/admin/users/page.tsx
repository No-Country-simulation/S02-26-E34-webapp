    ...
    return (
        <AdminLayout>
            <div className="bg-card border border-border rounded-2xl overflow-hidden flex flex-col h-[calc(100vh-250px)] min-h-125">
                {/* Toolbar */}
                <div className="p-4 sm:p-6 border-b border-white/5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div>
                ...
                <div className="flex-1 overflow-auto">
                    {isLoading ? (
                        ...
                        <div className="flex flex-col items-center justify-center p-12 text-center h-full">
                            ...
                            <p className="text-sm font-medium text-slate-400">No se encontraron usuarios que coincidan con la búsqueda.</p>
                        </div>
                    ) : (
                        <table className="w-full text-left text-sm whitespace-nowrap">
                            <thead className="bg-[#1a1736]/50 sticky top-0 z-10 backdrop-blur-sm">
                                <tr>
                                    <th className="px-6 py-4 font-semibold text-slate-300">Usuario</th>
                                    <th className="px-6 py-4 font-semibold text-slate-300">Estado</th>
                                    <th className="px-6 py-4 font-semibold text-slate-300">Rol</th>
                                    <th className="px-6 py-4 font-semibold text-slate-300">Registro</th>
                                    ...
                                    <tr key={user._id} className="hover:bg-white/[0.02] transition-colors group">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className={`text-xs font-semibold uppercase tracking-wider ${user.role === 'admin' ? 'text-[#3b2bee]' : 'text-slate-400'
                                                    }`}>
                                                    {user.role}
                                                </span>
                                            </div>
                                                    ...
                                                    <>
                                                        {user.verification_status === 'verified' && (
                                                            <button
                                                                ...
                                                                disabled={!!actionLoading}
                                                                className="w-8 h-8 rounded-lg bg-foreground/5 text-slate-300 hover:bg-[#3b2bee] hover:text-white flex items-center justify-center transition-colors tooltip-trigger relative"
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-card border border-border rounded-2xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col">
                        <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
                            ...
                            <button onClick={() => setEditingUser(null)} className="text-slate-500 hover:text-foreground transition-colors">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        <div className="p-6 space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full bg-black/20 border border-border rounded-xl px-4 py-2 text-foreground focus:outline-none focus:ring-1 focus:ring-[#3b2bee]/50"
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-slate-400 mb-1">Correo Electrónico (No editable)</label>
                                <input
                                    type="email"
                                    value={editFormData.email}
                                    disabled
                                    className="w-full bg-black/40 border border-white/5 rounded-xl px-4 py-2 text-slate-500 cursor-not-allowed"
                                />
                            </div>
                            <div>
                                ...
                                <select
                                    ...
                                    className="w-full bg-black/20 border border-border rounded-xl px-4 py-2 text-foreground focus:outline-none focus:ring-1 focus:ring-[#3b2bee]/50 appearance-none"
                                >
